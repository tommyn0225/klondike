...
local TABLEAU_GAP_FACEUP   = 32
local TABLEAU_GAP_FACEDOWN = 12
local WASTE_FAN_X          = 20
...
local function can_tab(c,d)  return (d and c.rank==d.rank-1 and opp_color(c.suit,d.suit))
                                or (not d and c.rank==13) end
local function can_found(c,d) return (d and c.suit==d.suit and c.rank==d.rank+1)
                                or (not d and c.rank==1) end
...
function Card:new(s,r,img) return setmetatable({suit=s,rank=r,img=img,faceUp=false,x=0,y=0,pile=nil},Card) end
function Card:draw() love.graphics.draw(self.faceUp and self.img or CARD_BACK, self.x, self.y) end

local Pile={}; Pile.__index=Pile
function Pile:new(kind,x,y) return setmetatable({kind=kind,x=x,y=y,cards={}},Pile) end
function Pile:top() return self.cards[#self.cards] end
function Pile:isEmpty() return #self.cards==0 end
function Pile:add(c) self.cards[#self.cards+1]=c; c.pile=self end
function Pile:remove_from(card)
  local idx; for i,c in ipairs(self.cards) do if c==card then idx=i break end end
  local mv={} for i=idx,#self.cards do mv[#mv+1]=self.cards[i] end
...
function Pile:draw()
  ...
  elseif self.kind=="waste" then
    -- draw only the last three cards
    local n=#self.cards; for i=math.max(1,n-2), n do self.cards[i]:draw() end
  else
    local c=self:top()
    if c then c:draw() else
...
local function layout_waste()
  ...
  for i,c in ipairs(waste.cards) do
    local slot=i-(n-2)       -- keep last three visible
...
local function new_game()
  ...
  dragging=nil; moveCount=0; startTime=love.timer.getTime()
  ...
  discard = {}   -- hidden stack for previous waste groups
  piles[#piles+1]=stock; piles[#piles+1]=waste

  for i=1,4 do
    local f=Pile:new("foundation", margin+(3+i)*(CARD_W+20), topY)
    foundations[#foundations+1]=f; piles[#piles+1]=f
  end
  for i=1,7 do
    local t=Pile:new("tableau", margin+(i-1)*(CARD_W+20), tabY)
    tableaus[#tableaus+1]=t; piles[#piles+1]=t
  end

  local deck={}
  for _,s in ipairs(SUITS) do for r=1,13 do deck[#deck+1]=Card:new(s,r,images[s][r]) end end
  shuffle(deck)

  for i,t in ipairs(tableaus) do
    for j=1,i do
      local c=table.remove(deck)
      t:add(c); c.x=t.x; c.y=t.y+(j-1)*(j==i and TABLEAU_GAP_FACEUP or TABLEAU_GAP_FACEDOWN)
      if j==i then c.faceUp=true end
    end
  end
  for _,c in ipairs(deck) do stock:add(c); c.x,c.y=stock.x,stock.y end
end
...
function love.mousepressed(x,y,b)
  ...
  if over_head(stock,x,y) then
    if stock:isEmpty() then
      -- recycle waste + discard back to stock
      for i=#waste.cards,1,-1 do
        local c=table.remove(waste.cards); c.faceUp=false
        stock:add(c); c.x,c.y=stock.x,stock.y
      end
      for i=#discard,1,-1 do
        local c=table.remove(discard); c.faceUp=false
        stock:add(c); c.x,c.y=stock.x,stock.y
      end
    else
      -- move current waste to discard, then draw 3
      for i=#waste.cards,1,-1 do table.insert(discard, table.remove(waste.cards)) end
      for i=1,math.min(3,#stock.cards) do
        local c=table.remove(stock.cards); c.faceUp=true; waste:add(c)
      end
      layout_waste()
    end
  ...
  -- start drag
  local c=card_at(x,y); if not c then return end
  if c.pile.kind=="waste" and c~=c.pile:top() then return end
  if c.pile.kind=="foundation" or not c.faceUp then return end

  dragging={cards=c.pile:remove_from(c), dx=x-c.x, dy=y-c.y, origin=c.pile}
  if dragging.origin.kind=="tableau" then layout_tableau(dragging.origin) else layout_waste() end
end
...
local function try_place(stack,dest)
  ...
  if dest.kind=="tableau" then
    if can_tab(first,dest:top()) then
      ...
      layout_tableau(dest); return true
    end
  elseif dest.kind=="foundation" and #stack==1 then
    if can_found(first,dest:top()) then
      dest:add(first); first.x,first.y=dest.x,dest.y; return true
    end
  end
  return false
end
...
function love.mousereleased(x,y,b)
  ...
  if dragging.origin.kind=="tableau" and not dragging.origin:isEmpty() then
    local t=dragging.origin:top(); if not t.faceUp then t.faceUp=true end
  end
  dragging=nil
end
...
function love.draw()
  ...
  love.graphics.rectangle("fill",btnNewGame.x,btnNewGame.y,
                           btnNewGame.w,btnNewGame.h,6,6)
  love.graphics.setColor(1,1,1)
  love.graphics.printf("New Game",btnNewGame.x,btnNewGame.y+10,
                       btnNewGame.w,"center")

  local win=true
  for _,f in ipairs(foundations) do if #f.cards<13 then win=false break end end
  if win then
